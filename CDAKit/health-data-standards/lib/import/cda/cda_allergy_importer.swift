...
class HDSImport_CDA_AllergyImporter: HDSImport_CDA_SectionImporter {
  ...
  override init(entry_finder: HDSImport_CDA_EntryFinder = HDSImport_CDA_EntryFinder(entry_xpath: "//cda:observation[cda:templateId/@root='2.16.840.1.113883.10.20.1.18']")) {
  ...
  override func create_entry(entry_element: XMLElement, nrh: HDSImport_CDA_NarrativeReferenceHandler = HDSImport_CDA_NarrativeReferenceHandler()) -> HDSAllergy? {
    
    if let allergy = super.create_entry(entry_element, nrh: nrh) as? HDSAllergy {

      extract_negation(entry_element, entry: allergy)

      if let types = extract_code(entry_element, code_xpath: type_xpath) {
        allergy.type = HDSCodedEntries(entries: types)
      }
      if let reactions = extract_code(entry_element, code_xpath: reaction_xpath) {
        allergy.reaction = HDSCodedEntries(entries: reactions)
      }
      if let severity = extract_code(entry_element, code_xpath: severity_xpath) {
        allergy.severity = HDSCodedEntries(entries: severity)
      }
